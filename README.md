...
2. Abra o **SQL Editor** e rode o conteúdo de [`supabase/schema.sql`](supabase/schema.sql) — isso cria as tabelas `produtos`, `vendas`, `despesas`, `categorias`, `tipos`, o bucket de Storage `produtos` (fotos) e libera acesso para a chave anônima (não há tela de login nesta v1).
   - Se você já tinha rodado uma versão anterior deste schema, rode em vez disso, na ordem: [`supabase/migration_002_categorias_fotos.sql`](supabase/migration_002_categorias_fotos.sql), [`supabase/migration_003_tipos.sql`](supabase/migration_003_tipos.sql) e [`supabase/migration_004_sku_nao_unico.sql`](supabase/migration_004_sku_nao_unico.sql) — cada um adiciona só o que é novo, sem dar erro de "já existe".
